<script lang="ts">
  ...
  import { daysInput, quantityInput } from "./inputs.svelte";

  let user = $state<api.User | null>(null);
  let secret = $state<api.UserSecret | null>(null);
  let dosage = $state<Partial<api.DosageSchedule> | null>(null);

  let promise = $state(new Promise(() => {}));
  onMount(() => {
    promise = Promise.all([
      api.currentUser().then((u) => (user = u)),
      api.currentUserSecret().then((s) => (secret = s.secret)),
      api.dosageSchedule().then((s) => (dosage = s.schedule ?? {})),
    ]);
  });
  ...
  $inspect({ user, secret, dosage });

  const setDosageSchedule = api.debounced(api.setDosageSchedule, { ignoreFirstRun: true });
  let dosageIsValid = $derived(dosage && dosage.deliveryMethod && dosage.dose && dosage.interval);
  let dosagePromise = $derived(
    dosageIsValid
      ? setDosageSchedule(dosage as api.DosageSchedule).then(() => true)
...
<main id="settings" class="container spaced-2">
  {#if !deliveryMethod}
    <article
  ...
  <PreferenceGroup name="Dosage">
    {#snippet misc()}
      {@render preferencesLoader(dosagePromise)}
    {/snippet}
    ...
    {#if dosage && deliveryMethod}
      <div transition:slide={{ duration: 200, axis: "y" }}>
        <Preference name="Dose">
          ...
          {/snippet}
          <input
            onquantitychange={({ detail: data }) => (dosage!.dose = data.quantity)}
            use:quantityInput={{ unit: deliveryMethod.units, initial: dosage.dose }}
          />
        </Preference>
        ...
        <Preference name="Interval">
          ...
          <input
            placeholder="1 week"
            ondayschange={({ detail: data }) => (dosage!.interval = data.days)}
            use:daysInput={{ initial: dosage.interval }}
          />
        </Preference>
        ...
        {#if deliveryMethod.patch}
          <div transition:slide={{ duration: 200, axis: "y" }}>
            <Preference name="Patch Change">
              ...
              {/snippet}
              <input type="number" min="0" max="7" bind:value={dosage.concurrence} />
            </Preference>
          </div>
        {/if}
      </div>
...
{#snippet preferencesLoader(promise: Promise<any>)}
  {#await promise}
    <span aria-busy="true"></span>
  {:then saved}
    {#if saved}
